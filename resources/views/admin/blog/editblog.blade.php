...
@section('content')
  <h1 class="my-3">Edit Blog <i class="fa-solid fa-person fa-xs" data-fa-transform="down-2"></i></h1>
  <hr/>
  <form action="/blog/edit/{{$blog->id}}" class="row g-3 mb-2" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    <div class="col-12">
      <label for="title" class="form-label">Judul Blog</label>
      <input type="text" name="title" class="form-control" placeholder="Judul Blog" aria-label="title"aria-describedby="titleHelpBlock" value="{{$blog->title}}">
    ...
    <div class="col-12">
      ...
      <textarea class="form-control" id="" name="content" id="content" aria-label="content" aria-describedby="contentHelpBlock">{{$blog->content}}</textarea>
